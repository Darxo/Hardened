{	// Variables
	...
	// World
		ContractScalingBase = 1.0,		// This contract scaling is happening from day one. This scales additively with PerReputation scaling
...
{	// Functions
	...
	::MSU.Table.merge(::Hardened.Global, {
		...
		getWorldDifficultyMult = function() {
			local ret = ::Hardened.Global.WorldScalingBase + ::World.getTime().Days * ::Hardened.Global.WorldScalingPerDay;
		...
		getWorldContractMult = function() {
			local ret = ::Hardened.Global.ContractScalingBase + ::World.Assets.getBusinessReputation() * ::Hardened.Global.ContractScalingPerReputation;
