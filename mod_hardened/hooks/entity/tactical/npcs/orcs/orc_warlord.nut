...
::Hardened.HooksMod.hook("scripts/entity/tactical/enemies/orc_warlord", function(q) {
	q.create = @(__original) function()
	{
		...
		// We make it so regular Orc Warlords no longer use a unique name, so they don't get confused for Champions
...
// New Functions
	...
	{
		this.getItems().getAppearance().Body = "bust_orc_04_body";
	...
	q.HD_onInitStatsAndSkills <- function()
	{
		...
		this.getSkills().add(::new("scripts/skills/actives/line_breaker"));
		this.getSkills().add(::new("scripts/skills/actives/warcry"));
	}
