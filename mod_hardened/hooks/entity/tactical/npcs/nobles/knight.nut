...
::Hardened.HooksMod.hook("scripts/entity/tactical/humans/knight", function(q) {
	...
	q.makeMiniboss = @() { function makeMiniboss()
	{
		...
		// Knight Champions never spawn with named helmets as we always want to preserve their faction themed helmet
		...
		{
			this.getItems().equip(::new("scripts/items/" + ::Const.Items.NamedShields[::Math.rand(0, ::Const.Items.NamedShields.len() - 1)]));
		}
		else if (r == 3)
		{
