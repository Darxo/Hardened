...
// Brigand Marauder - Balanced Tough - Tier 3 (Reforged: Brigand Marauder)
::Hardened.HooksMod.hook("scripts/entity/tactical/enemies/rf_bandit_marauder_tough", function(q) {
	q.create = @(__original) function()
	{
		this.m.Bodies = ::Const.Bodies.Thick;	// Reforged: ::Const.Bodies.AllMale

		// Switcheroo the RF_BanditMarauder ID into that of HD_BanditMarauder, to make Reforged assign the ID, that we want
		// We must do this via switcheroo, so that the name assignment works correctly
		local oldRF_BanditMarauder = ::Const.EntityType.RF_BanditMarauder;
		::Const.EntityType.RF_BanditMarauder = ::Const.EntityType.HD_BanditMarauder;
		__original();
		::Const.EntityType.RF_BanditMarauder = oldRF_BanditMarauder;
...
// New Functions
	...
	q.HD_onInitStatsAndSkills <- function()
	{
		...
		b.setValues(::Const.Tactical.Actor.RF_BanditMarauderTough);
