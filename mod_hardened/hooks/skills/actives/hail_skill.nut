::Hardened.HooksMod.hook("scripts/skills/actives/hail_skill", function(q) {
	...
	q.getTooltip = @(__original) function()
	{
		...
		{
			if (entry.id == 9 && entry.icon == "ui/icons/special.png")
			{
				entry.icon = "ui/icons/chance_to_hit_head.png";
			}
	...
	// This is an exact copy of the "onUse" implementation in cascade_skill
	...
	q.onAnySkillUsed = @() function( _skill, _targetEntity, _properties )
	{
		if (_skill == this)
		{
			_properties.HitChance[::Const.BodyPart.Head] += 100.0;		// Same as Vanilla
