// scripts/entity/tactical/humans/noble_billman
{
	// Mandatory stats
	::Const.Tactical.Actor.Billman.XP = 240 * ::Hardened.Global.FactionExperience.Nobles;
	::Const.Tactical.Actor.Billman.ActionPoints = 9;
	::Const.Tactical.Actor.Billman.Hitpoints = 70;
	::Const.Tactical.Actor.Billman.Bravery = 60;
	::Const.Tactical.Actor.Billman.Stamina = 100;
	::Const.Tactical.Actor.Billman.MeleeSkill = 75;
	::Const.Tactical.Actor.Billman.RangedSkill = 50;
	::Const.Tactical.Actor.Billman.MeleeDefense = 10;
	::Const.Tactical.Actor.Billman.RangedDefense = 10;
	::Const.Tactical.Actor.Billman.Initiative = 100;
}

// scripts/entity/tactical/humans/noble_arbalester
{
	// Mandatory stats
	::Const.Tactical.Actor.Arbalester.XP = 240 * ::Hardened.Global.FactionExperience.Nobles;
	::Const.Tactical.Actor.Arbalester.ActionPoints = 9;
	::Const.Tactical.Actor.Arbalester.Hitpoints = 70;
	::Const.Tactical.Actor.Arbalester.Bravery = 60;
	::Const.Tactical.Actor.Arbalester.Stamina = 100;
	::Const.Tactical.Actor.Arbalester.MeleeSkill = 55;
	::Const.Tactical.Actor.Arbalester.RangedSkill = 75;
	::Const.Tactical.Actor.Arbalester.MeleeDefense = 0;
	::Const.Tactical.Actor.Arbalester.RangedDefense = 10;
	::Const.Tactical.Actor.Arbalester.Initiative = 110;

	// Optional Stats
	::Const.Tactical.Actor.Arbalester.Vision <- 8;			// Vanilla: 7
}

// scripts/entity/tactical/humans/noble_footman
{
	// Mandatory stats
	::Const.Tactical.Actor.Footman.XP = 240 * ::Hardened.Global.FactionExperience.Nobles;
	::Const.Tactical.Actor.Footman.ActionPoints = 9;
	::Const.Tactical.Actor.Footman.Hitpoints = 85;
	::Const.Tactical.Actor.Footman.Bravery = 65;
	::Const.Tactical.Actor.Footman.Stamina = 120;
	::Const.Tactical.Actor.Footman.MeleeSkill = 70;
	::Const.Tactical.Actor.Footman.RangedSkill = 50;
	::Const.Tactical.Actor.Footman.MeleeDefense = 20;
	::Const.Tactical.Actor.Footman.RangedDefense = 10;
	::Const.Tactical.Actor.Footman.Initiative = 110;
}

// scripts/entity/tactical/humans/rf_squire
{
	// Mandatory stats
	::Const.Tactical.Actor.RF_Squire.XP = 350 * ::Hardened.Global.FactionExperience.Nobles;
	::Const.Tactical.Actor.RF_Squire.ActionPoints = 9;
	::Const.Tactical.Actor.RF_Squire.Hitpoints = 70;
	::Const.Tactical.Actor.RF_Squire.Bravery = 60;
	::Const.Tactical.Actor.RF_Squire.Stamina = 120;
	::Const.Tactical.Actor.RF_Squire.MeleeSkill = 70;
	::Const.Tactical.Actor.RF_Squire.RangedSkill = 50;
	::Const.Tactical.Actor.RF_Squire.MeleeDefense = 10;
	::Const.Tactical.Actor.RF_Squire.RangedDefense = 15;
	::Const.Tactical.Actor.RF_Squire.Initiative = 120;
}

// scripts/entity/tactical/humans/standard_bearer
{
	// Mandatory stats
	::Const.Tactical.Actor.StandardBearer.XP = 300 * ::Hardened.Global.FactionExperience.Nobles;
	::Const.Tactical.Actor.StandardBearer.ActionPoints = 9;
	::Const.Tactical.Actor.StandardBearer.Hitpoints = 70;
	::Const.Tactical.Actor.StandardBearer.Bravery = 110;
	::Const.Tactical.Actor.StandardBearer.Stamina = 115;
	::Const.Tactical.Actor.StandardBearer.MeleeSkill = 65;
	::Const.Tactical.Actor.StandardBearer.RangedSkill = 40;
	::Const.Tactical.Actor.StandardBearer.MeleeDefense = 10;
	::Const.Tactical.Actor.StandardBearer.RangedDefense = 10;
	::Const.Tactical.Actor.StandardBearer.Initiative = 105;
}

// scripts/entity/tactical/humans/noble_sergeant
{
	// Mandatory stats
	::Const.Tactical.Actor.Sergeant.XP = 350 * ::Hardened.Global.FactionExperience.Nobles;
	::Const.Tactical.Actor.Sergeant.ActionPoints = 9;
	::Const.Tactical.Actor.Sergeant.Hitpoints = 120;
	::Const.Tactical.Actor.Sergeant.Bravery = 90;
	::Const.Tactical.Actor.Sergeant.Stamina = 130;
	::Const.Tactical.Actor.Sergeant.MeleeSkill = 75;
	::Const.Tactical.Actor.Sergeant.RangedSkill = 60;
	::Const.Tactical.Actor.Sergeant.MeleeDefense = 0;
	::Const.Tactical.Actor.Sergeant.RangedDefense = 30;
	::Const.Tactical.Actor.Sergeant.Initiative = 110;
}

// scripts/entity/tactical/humans/rf_arbalester_heavy
{
	// Mandatory stats
	::Const.Tactical.Actor.RF_ArbalesterHeavy.XP = 400 * ::Hardened.Global.FactionExperience.Nobles;
	::Const.Tactical.Actor.RF_ArbalesterHeavy.ActionPoints = 9;
	::Const.Tactical.Actor.RF_ArbalesterHeavy.Hitpoints = 80;
	::Const.Tactical.Actor.RF_ArbalesterHeavy.Bravery = 60;
	::Const.Tactical.Actor.RF_ArbalesterHeavy.Stamina = 110;
	::Const.Tactical.Actor.RF_ArbalesterHeavy.MeleeSkill = 55;
	::Const.Tactical.Actor.RF_ArbalesterHeavy.RangedSkill = 75;
	::Const.Tactical.Actor.RF_ArbalesterHeavy.MeleeDefense = 0;
	::Const.Tactical.Actor.RF_ArbalesterHeavy.RangedDefense = 15;
	::Const.Tactical.Actor.RF_ArbalesterHeavy.Initiative = 120;

	// Optional Stats
	::Const.Tactical.Actor.RF_ArbalesterHeavy.Vision <- 8;
}

// scripts/entity/tactical/humans/rf_billman_heavy
{
	// Mandatory stats
	::Const.Tactical.Actor.RF_BillmanHeavy.XP = 400 * ::Hardened.Global.FactionExperience.Nobles;
	::Const.Tactical.Actor.RF_BillmanHeavy.ActionPoints = 9;
	::Const.Tactical.Actor.RF_BillmanHeavy.Hitpoints = 80;
	::Const.Tactical.Actor.RF_BillmanHeavy.Bravery = 70;
	::Const.Tactical.Actor.RF_BillmanHeavy.Stamina = 120;
	::Const.Tactical.Actor.RF_BillmanHeavy.MeleeSkill = 85;
	::Const.Tactical.Actor.RF_BillmanHeavy.RangedSkill = 50;
	::Const.Tactical.Actor.RF_BillmanHeavy.MeleeDefense = 15;
	::Const.Tactical.Actor.RF_BillmanHeavy.RangedDefense = 15;
	::Const.Tactical.Actor.RF_BillmanHeavy.Initiative = 110;
}

// scripts/entity/tactical/humans/rf_footman_heavy
{
	// Mandatory stats
	::Const.Tactical.Actor.RF_FootmanHeavy.XP = 400 * ::Hardened.Global.FactionExperience.Nobles;
	::Const.Tactical.Actor.RF_FootmanHeavy.ActionPoints = 9;
	::Const.Tactical.Actor.RF_FootmanHeavy.Hitpoints = 100;
	::Const.Tactical.Actor.RF_FootmanHeavy.Bravery = 75;
	::Const.Tactical.Actor.RF_FootmanHeavy.Stamina = 150;
	::Const.Tactical.Actor.RF_FootmanHeavy.MeleeSkill = 75;
	::Const.Tactical.Actor.RF_FootmanHeavy.RangedSkill = 50;
	::Const.Tactical.Actor.RF_FootmanHeavy.MeleeDefense = 30;
	::Const.Tactical.Actor.RF_FootmanHeavy.RangedDefense = 20;
	::Const.Tactical.Actor.RF_FootmanHeavy.Initiative = 120;
}

// scripts/entity/tactical/humans/noble_greatsword
{
	// Mandatory stats
	::Const.Tactical.Actor.Greatsword.XP = 500 * ::Hardened.Global.FactionExperience.Nobles;
	::Const.Tactical.Actor.Greatsword.ActionPoints = 9;
	::Const.Tactical.Actor.Greatsword.Hitpoints = 90;
	::Const.Tactical.Actor.Greatsword.Bravery = 70;
	::Const.Tactical.Actor.Greatsword.Stamina = 150;
	::Const.Tactical.Actor.Greatsword.MeleeSkill = 90;
	::Const.Tactical.Actor.Greatsword.RangedSkill = 40;
	::Const.Tactical.Actor.Greatsword.MeleeDefense = 25;
	::Const.Tactical.Actor.Greatsword.RangedDefense = 15;
	::Const.Tactical.Actor.Greatsword.Initiative = 115;
}

// scripts/entity/tactical/humans/rf_fencer
{
	// Mandatory stats
	::Const.Tactical.Actor.RF_Fencer.XP = 500 * ::Hardened.Global.FactionExperience.Nobles;
	::Const.Tactical.Actor.RF_Fencer.ActionPoints = 9;
	::Const.Tactical.Actor.RF_Fencer.Hitpoints = 55;
	::Const.Tactical.Actor.RF_Fencer.Bravery = 70;
	::Const.Tactical.Actor.RF_Fencer.Stamina = 150;
	::Const.Tactical.Actor.RF_Fencer.MeleeSkill = 100;
	::Const.Tactical.Actor.RF_Fencer.RangedSkill = 50;
	::Const.Tactical.Actor.RF_Fencer.MeleeDefense = 20;
	::Const.Tactical.Actor.RF_Fencer.RangedDefense = 10;
	::Const.Tactical.Actor.RF_Fencer.Initiative = 160;
}

// scripts/entity/tactical/humans/rf_man_at_arms
{
	// Mandatory stats
	::Const.Tactical.Actor.RF_ManAtArms.XP = 500 * ::Hardened.Global.FactionExperience.Nobles;
	::Const.Tactical.Actor.RF_ManAtArms.ActionPoints = 9;
	::Const.Tactical.Actor.RF_ManAtArms.Hitpoints = 100;
	::Const.Tactical.Actor.RF_ManAtArms.Bravery = 70;
	::Const.Tactical.Actor.RF_ManAtArms.Stamina = 150;
	::Const.Tactical.Actor.RF_ManAtArms.MeleeSkill = 85;
	::Const.Tactical.Actor.RF_ManAtArms.RangedSkill = 50;
	::Const.Tactical.Actor.RF_ManAtArms.MeleeDefense = 30;
	::Const.Tactical.Actor.RF_ManAtArms.RangedDefense = 20;
	::Const.Tactical.Actor.RF_ManAtArms.Initiative = 110;
}

// scripts/entity/tactical/humans/rf_herald
{
	// Mandatory stats
	::Const.Tactical.Actor.RF_Herald.XP = 450 * ::Hardened.Global.FactionExperience.Nobles;
	::Const.Tactical.Actor.RF_Herald.ActionPoints = 9;
	::Const.Tactical.Actor.RF_Herald.Hitpoints = 80;
	::Const.Tactical.Actor.RF_Herald.Bravery = 150;
	::Const.Tactical.Actor.RF_Herald.Stamina = 125;
	::Const.Tactical.Actor.RF_Herald.MeleeSkill = 75;
	::Const.Tactical.Actor.RF_Herald.RangedSkill = 50;
	::Const.Tactical.Actor.RF_Herald.MeleeDefense = 20;
	::Const.Tactical.Actor.RF_Herald.RangedDefense = 20;
	::Const.Tactical.Actor.RF_Herald.Initiative = 115;
}

// scripts/entity/tactical/humans/rf_heralds_bodyguard
{
	// Mandatory stats
	::Const.Tactical.Actor.RF_HeraldsBodyguard.XP = 450 * ::Hardened.Global.FactionExperience.Nobles;
	::Const.Tactical.Actor.RF_HeraldsBodyguard.ActionPoints = 9;
	::Const.Tactical.Actor.RF_HeraldsBodyguard.Hitpoints = 120;
	::Const.Tactical.Actor.RF_HeraldsBodyguard.Bravery = 90;
	::Const.Tactical.Actor.RF_HeraldsBodyguard.Stamina = 140;
	::Const.Tactical.Actor.RF_HeraldsBodyguard.MeleeSkill = 75;
	::Const.Tactical.Actor.RF_HeraldsBodyguard.RangedSkill = 60;
	::Const.Tactical.Actor.RF_HeraldsBodyguard.MeleeDefense = 15;
	::Const.Tactical.Actor.RF_HeraldsBodyguard.RangedDefense = 0;
	::Const.Tactical.Actor.RF_HeraldsBodyguard.Initiative = 130;
}

// scripts/entity/tactical/humans/rf_marshal
{
	// Mandatory stats
	::Const.Tactical.Actor.RF_Marshal.XP = 600 * ::Hardened.Global.FactionExperience.Nobles;
	::Const.Tactical.Actor.RF_Marshal.ActionPoints = 9;
	::Const.Tactical.Actor.RF_Marshal.Hitpoints = 150;
	::Const.Tactical.Actor.RF_Marshal.Bravery = 120;
	::Const.Tactical.Actor.RF_Marshal.Stamina = 140;
	::Const.Tactical.Actor.RF_Marshal.MeleeSkill = 85;
	::Const.Tactical.Actor.RF_Marshal.RangedSkill = 60;
	::Const.Tactical.Actor.RF_Marshal.MeleeDefense = 10;
	::Const.Tactical.Actor.RF_Marshal.RangedDefense = 50;
	::Const.Tactical.Actor.RF_Marshal.Initiative = 120;
}

// scripts/entity/tactical/humans/knight
{
	// Mandatory stats
	::Const.Tactical.Actor.Knight.XP = 700 * ::Hardened.Global.FactionExperience.Nobles;
	::Const.Tactical.Actor.Knight.ActionPoints = 9;
	::Const.Tactical.Actor.Knight.Hitpoints = 150;
	::Const.Tactical.Actor.Knight.Bravery = 70;
	::Const.Tactical.Actor.Knight.Stamina = 150;
	::Const.Tactical.Actor.Knight.MeleeSkill = 95;
	::Const.Tactical.Actor.Knight.RangedSkill = 40;
	::Const.Tactical.Actor.Knight.MeleeDefense = 30;
	::Const.Tactical.Actor.Knight.RangedDefense = 10;
	::Const.Tactical.Actor.Knight.Initiative = 115;
}

// scripts/entity/tactical/humans/rf_knight_anointed
{
	// Mandatory stats
	::Const.Tactical.Actor.RF_KnightAnointed.XP = 1000 * ::Hardened.Global.FactionExperience.Nobles;
	::Const.Tactical.Actor.RF_KnightAnointed.ActionPoints = 9;
	::Const.Tactical.Actor.RF_KnightAnointed.Hitpoints = 170;
	::Const.Tactical.Actor.RF_KnightAnointed.Bravery = 80;
	::Const.Tactical.Actor.RF_KnightAnointed.Stamina = 170;
	::Const.Tactical.Actor.RF_KnightAnointed.MeleeSkill = 100;
	::Const.Tactical.Actor.RF_KnightAnointed.RangedSkill = 40;
	::Const.Tactical.Actor.RF_KnightAnointed.MeleeDefense = 40;
	::Const.Tactical.Actor.RF_KnightAnointed.RangedDefense = 15;
	::Const.Tactical.Actor.RF_KnightAnointed.Initiative = 125;
}
